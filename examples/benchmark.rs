...
use bitonic_sorter::utils::{is_sorted_asceding, new_u32_vec};
...
fn main() {
    if let Some(n) = env::args().nth(1) {
        let bits = u32::from_str(&n).expect("error parsing argment");
        run_sorts(bits);
    } else {
        eprintln!(
...
fn run_sorts(bits: u32) {
    ...
    let par_duration = timed_sort(&par_sort, len, "para_sort");
